...
------------------------
Pretends to be 3 "high-velocity" branch servers (JioHotstar-Mumbai,
JioHotstar-Delhi, JioHotstar-Bangalore). Each one is a tiny TCP server that,
...
BRANCHES = [
    ("jiohotstar-mumbai", 9001),
    ("jiohotstar-delhi", 9002),
    ("jiohotstar-bangalore", 9003),
]
...
MESSAGE_TEMPLATES = {
    "INFO": [
        "Stream#{oid} started successfully",
        "User#{oid} subscription renewed",
        "Stream#{oid} switched to 1080p",
        "Content#{oid} added to watchlist",
    ],
    "WARNING": [
        "Stream#{oid} buffering above threshold",
        "CDN response time high for Stream#{oid}",
        "High concurrent viewer count near Stream#{oid}",
    ],
    "ERROR": [
        "Payment gateway timeout for Subscription#{oid}",
        "Stream#{oid} failed due to CDN unavailability",
        "DRM license fetch failed for Stream#{oid}",
    ],
    "DEBUG": [
        "Cache miss while fetching metadata for Content#{oid}",
        "Retrying DB write for Session#{oid}",
    ],
}
